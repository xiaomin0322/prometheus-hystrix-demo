...
public class HystrixPrometheusMetricsPublisherThreadPool implements HystrixMetricsPublisherThreadPool {
    ...
    public void initialize() {
        ...
        //下划线太多了，没效果
        addGauge("count_thread_pool_rejected", currentStateDoc, new Callable<Number>() {
            @Override
            public Number call() {
                return  metrics.getCumulativeCountThreadsRejected();
            }
        });
        //这个有效果
        addGauge("thread_pool_rejected", currentStateDoc, new Callable<Number>() {
            @Override
            public Number call() {
                return  metrics.getCumulativeCountThreadsRejected();
            }
